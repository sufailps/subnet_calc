...
def main_fun():
    ...
    #broad cast
    inverted_subnet=''.join(str("0"*subnet+"1"*(32-subnet)))
    inverted= re.findall('........',inverted_subnet)
    invert_lst = []
    for i in inverted:
        invert_lst.append(int(i,2)) #convert to decimal
    
    brodcast_lst = []
    for i in range(len(iplist)):
        brodcast_lst.append(network_address[i] | invert_lst[i]) #decimal version  (OR)
    brodacast_address = ".".join(map(str, brodcast_lst))
    #to find no of ip address   = 2^n
    ...
    Label(win, text=brodacast_address).grid(row=34,column=2)
